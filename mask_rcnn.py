...
class MaskRCNN:
    ...
    def draw_object_info(self, bgr_frame, depth_frame):
        ...
        for box, class_id, obj_center in zip(self.obj_boxes, self.obj_classes, self.obj_centers):
            ...
            cv2.putText(bgr_frame, "{} cm".format(depth_mm / 100), (x + 5, y + 60), 0, 1.0, (255, 255, 255), 2)
            cv2.rectangle(bgr_frame, (x, y), (x2, y2), color, 1)

            distance = depth_mm / 100
            middle_range = 45
            if distance < 25:
                speak = wincom.Dispatch("SAPI.SpVoice")
                if cx < frame_midpoint - middle_range:
                    direction = "left"
                    move_direction = "right"
                    cv2.putText(bgr_frame, direction, (x + 5, y + 95), 0, 0.8, (255, 255, 255), 2)
                    answer = f"{class_name} at your {direction} side is too near, please move to {move_direction} side"

                elif cx > frame_midpoint + middle_range:
                    direction = "right"
                    move_direction = "left"
                    cv2.putText(bgr_frame, direction, (x + 5, y + 95), 0, 0.8, (255, 255, 255), 2)
                    answer = f"{class_name} at your {direction} side is too near, please move to {move_direction} side"
                    ...
                    move_direction = "back"
                    cv2.putText(bgr_frame, direction, (x + 5, y + 95), 0, 0.8, (255, 255, 255), 2)
                    answer = f"{class_name} at your {direction} is too near, please go side way"
                speak.Speak(answer)
